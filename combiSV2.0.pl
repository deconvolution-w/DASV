...
if ($input_pbsv ne "")
{
    while (my $line = <INPUT_PBSV>)
    {
        ...
        if ($first_nuc ne "#")
        {
            ...
            if ($type ne "BND" && $type ne "cnv" && $list[6] eq "PASS" && ($type ne "DUP" || $high_recall eq "2" || $haplo eq "1/1") && ($HAP[1] >= $min_coverage ||
               ($HAP[1] >= $min_coverage-1 && $HAP[1]/$HAP[2] > 0.3)) && $haplo ne "0/0" && $HAP[1]/$HAP[2] > 0.3 && ($SVLEN eq "." || $SVLEN > 35))
            {                  
...
CUTESV: while (my $line = <INPUT_CUTESV>)
    {
        ...
        if ($first ne "#")
        {
            ...
            if ($list[6] eq "PASS" && ($length eq "." || $length > 35) && ($HAP[2] >= $min_coverage || ($HAP[2] >= $min_coverage-1 && $HAP[2]/($HAP[2]+$HAP[1]) > 0.3))
                && $HAP[2]/($HAP[2]+$HAP[1]) > 0.12 && $type ne "BND")
            {               
                my $prev_match = "";
...
NANOVAR: while (my $line = <INPUT_NANOVAR>)
    {
        ...
        if ($first_nuc ne "#")
        {
            ...
            if (($HAP2[1] >= $min_coverage || ($HAP2[1] >= $min_coverage-1 && $HAP2[0]/($HAP_REF[0]+$HAP2[0]) > 0.3)) && $list[6] eq "PASS" && $type ne "."
                && $type ne "BND" && ($HAP2[0]/($HAP_REF[0]+$HAP2[0]) > 0.12 || $HAP_REF[0] eq ".") && ($length eq "." || $length > 35) && ($HAP[0] ne "./." || $count_tools < 4))
            {    
                my @list2 = split /\t/, $SVs{$chr}{$list[1]};
...
SVIM: while (my $line = <INPUT_SVIM>)
    {
        ...
        if ($first_nuc ne "#")
        {
            ...
            if ($list[6] eq "PASS" && ($support >= $min_coverage || ($support >= $min_coverage-1 && $ratio > 0.3)) && $type ne "BND" && $type ne "DUP" && $hapi ne "0/0" && $hapi ne "./." &&
                ($ratio > 0.2 || $HAP_INFO[1] eq "CN" || $HAP[1] eq ".") && ($length eq "." || $length > 35))
            {          
...
NANOSV: while (my $line = <INPUT_NANOSV>)
    {
        ...
        if ($first_nuc ne "#")
        {
            ...
            if (($HAP2[1] >= $min_coverage+1 ) && ($HAP2[0]/($HAP_REF[0]+$HAP2[0]) > 0.2 || $HAP_REF[0] eq ".") && $hapi ne "0/0" && ($length eq "." || $length > 35) && $qual_check ne "no")
            {                
                my $prev_match_pbsv = "";
                my $prev_match_cutesv = "";
                my $prev_match_sniffles = "";
